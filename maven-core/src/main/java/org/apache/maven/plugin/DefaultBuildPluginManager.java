...
public class DefaultBuildPluginManager implements BuildPluginManager {
  ...
  @Inject private MojoExecutionScope scope;

  private MojoExecutionListener mojoExecutionListener;

  @Inject
  public void
  setMojoExecutionListeners(final List<MojoExecutionListener> listeners) {
   ...
   *     repositories.
   * @throws PluginResolutionException The plugin could be found but could not
   *     be resolved.
  ...
  public PluginDescriptor loadPlugin(Plugin plugin,
    ...
    return mavenPluginManager.getPluginDescriptor(plugin, repositories,
                                                  session);
  }
  ...
  public void executeMojo(MavenSession session, MojoExecution mojoExecution)
    ...
    try {
      pluginRealm =
          getPluginRealm(session, mojoDescriptor.getPluginDescriptor());
    ...
    try {
      ...
      // NOTE: DuplicateArtifactAttachmentException is currently unchecked, so
      // be careful removing this try/catch! This is necessary to avoid creating
      // compatibility problems for existing plugins that use
      // MavenProjectHelper.attachArtifact(..).
      try {
        MojoExecutionEvent mojoExecutionEvent =
            new MojoExecutionEvent(session, project, mojoExecution, mojo);
      ...
      Exception wrapper = new PluginContainerException(
          mojoDescriptor, pluginRealm, os.toString(), e);
      ...
      ByteArrayOutputStream os = new ByteArrayOutputStream(1024);
      PrintStream ps = new PrintStream(os);
      ps.println("An API incompatibility was encountered while executing " +
      ...
      ByteArrayOutputStream os = new ByteArrayOutputStream(1024);
      PrintStream ps = new PrintStream(os);
      ps.println("A type incompatibility occurred while executing " +
                 mojoDescriptor.getId() + ": " + e.getMessage());
      pluginRealm.display(ps);

      throw new PluginExecutionException(mojoExecution, project, os.toString(),
                                         e);
    } catch (RuntimeException e) {
      mojoExecutionListener.afterExecutionFailure(
          new MojoExecutionEvent(session, project, mojoExecution, mojo, e));
  ...
  public MojoDescriptor getMojoDescriptor(Plugin plugin, String goal,
      ...
      throws PluginNotFoundException, PluginResolutionException,
             ...
             InvalidPluginDescriptorException {
    return mavenPluginManager.getMojoDescriptor(plugin, goal, repositories,
                                                session);
  }
}
