...
public class DefaultPluginDescriptorCache implements PluginDescriptorCache {
  ...
  public void flush() { descriptors.clear(); }
  ...
  protected static PluginDescriptor clone(PluginDescriptor original) {
    ...
    if (original != null) {
      ...
      clone.setPluginArtifact(
          ArtifactUtils.copyArtifactSafe(original.getPluginArtifact()));
  ...
  private static List<ComponentDescriptor<?>>
  clone(List<MojoDescriptor> mojos, PluginDescriptor pluginDescriptor) {
  ...
  private static final class CacheKey implements Key {
    ...
    CacheKey(Plugin plugin, List<RemoteRepository> repositories,
             RepositorySystemSession session) {
