...
public class ProjectSorter {
  ...
  // relation, but also for <parent>, <build><plugin>, <reports>. We need
  // multiple DAG's since a DAG can only handle 1 type of relationship properly.
  ...
  // although that seems to work fine. We need to take versions and lifecycle
  // into account.
    ...
    for (MavenProject project : projects) {
      ...
      if (conflictingProject != null) {
        throw new DuplicateProjectException(
            projectId, conflictingProject.getFile(), project.getFile(),
            "Project '" + projectId + "' is duplicated in the reactor");
      }

      String projectKey = ArtifactUtils.versionlessKey(project.getGroupId(),
    ...
    for (Vertex projectVertex : dag.getVertices()) {
      ...
      if (parent != null) {
        ...
        addEdge(projectMap, vertexMap, null, projectVertex, parent.getGroupId(),
                parent.getArtifactId(), parent.getVersion(), true, false);
      }
      ...
      if (buildPlugins != null) {
        for (Plugin plugin : buildPlugins) {
          ...
          for (Dependency dependency : plugin.getDependencies()) {
            addEdge(projectMap, vertexMap, project, projectVertex,
                    dependency.getGroupId(), dependency.getArtifactId(),
                    dependency.getVersion(), false, true);
          }
        }
      }
      ...
      if (buildExtensions != null) {
        for (Extension extension : buildExtensions) {
          addEdge(projectMap, vertexMap, project, projectVertex,
                  ...
                  extension.getVersion(), false, true);
        }
      }
    }
  ...
  private void addEdge(Map<String, MavenProject> projectMap,
                       Map<String, Map<String, Vertex>> vertexMap,
                       MavenProject project, Vertex projectVertex,
  ...
  private void addEdge(Vertex fromVertex, Vertex toVertex,
                       ...
                       Map<String, MavenProject> projectMap, boolean force,
                       boolean safe) throws CycleDetectedException {
  ...
  public MavenProject getTopLevelProject() {
    if (topLevelProject == null) {
      for (Iterator<MavenProject> i = sortedProjects.iterator();
           i.hasNext() && (topLevelProject == null);) {
  ...
  public List<MavenProject> getSortedProjects() { return sortedProjects; }

  public boolean hasMultipleProjects() { return sortedProjects.size() > 1; }
  ...
  public static String getId(MavenProject project) {
    return ArtifactUtils.key(project.getGroupId(), project.getArtifactId(),
                             project.getVersion());
  }

  public DAG getDAG() { return dag; }

  public Map<String, MavenProject> getProjectMap() { return projectMap; }
}
