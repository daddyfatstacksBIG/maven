...
public class DefaultLifecycleExecutor implements LifecycleExecutor {
  ...
  @Inject private LifeCyclePluginAnalyzer lifeCyclePluginAnalyzer;
  ...
  @Inject private MojoExecutor mojoExecutor;
  ...
  // These methods deal with construction intact Plugin object that look like
  // they come from a standard <plugin/> block in a Maven POM. We have to do
  // some wiggling to pull the sources of information together and this really
  // shows the problem of constructing a sensible default configuration but it's
  // all encapsulated here so it appears normalized to the POM builder.

  // We are going to take the project packaging and find all plugin in the
  // default lifecycle and create fully populated Plugin objects, including
  // executions with goals and default configuration taken from the plugin.xml
  // inside a plugin.
  //
  // TODO This whole method could probably removed by injecting
  // lifeCyclePluginAnalyzer straight into client site.
  // TODO But for some reason the whole plexus appcontext refuses to start when
  // I try this.

  public Set<Plugin> getPluginsBoundByDefaultToAllLifecycles(String packaging) {
    return lifeCyclePluginAnalyzer.getPluginsBoundByDefaultToAllLifecycles(
        packaging);
  }
  ...
  calculateExecutionPlan(MavenSession session, boolean setup, String... tasks)
    ...
    List<TaskSegment> taskSegments =
        lifecycleTaskSegmentCalculator.calculateTaskSegments(
            session, Arrays.asList(tasks));
  ...
  public void calculateForkedExecutions(MojoExecution mojoExecution,
      ...
      throws MojoNotFoundException, PluginNotFoundException,
             ...
             PluginVersionResolutionException {
    lifecycleExecutionPlanCalculator.calculateForkedExecutions(mojoExecution,
                                                               session);
  }
  ...
  public List<MavenProject> executeForkedExecutions(MojoExecution mojoExecution,
    ...
    return mojoExecutor.executeForkedExecutions(
        mojoExecution, session, new ProjectIndex(session.getProjects()));
  }
}
