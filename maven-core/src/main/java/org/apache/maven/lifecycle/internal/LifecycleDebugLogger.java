...
public class LifecycleDebugLogger {
  ...
  public LifecycleDebugLogger(Logger logger) { this.logger = logger; }
  ...
  public void debugReactorPlan(ProjectBuildList projectBuilds) {
    ...
    for (Iterator<ProjectSegment> it = projectBuilds.iterator();
         it.hasNext();) {
      ...
      logger.debug("Style:   " + (projectBuild.getTaskSegment().isAggregating()
                                      ? "Aggregating"
                                      : "Regular"));
  ...
  public void debugProjectPlan(MavenProject currentProject,
    ...
    logger.debug("Repositories (dependencies): " +
                 currentProject.getRemoteProjectRepositories());
    logger.debug("Repositories (plugins)     : " +
                 currentProject.getRemotePluginRepositories());
  ...
  private void debugMojoExecution(MojoExecution mojoExecution) {
    ...
    if (!forkedExecutions.isEmpty()) {
      for (Map.Entry<String, List<MojoExecution>> fork :
        ...
        logger.debug("--- init fork of " + fork.getKey() + " for " +
                     mojoExecId + " ---");
        ...
        logger.debug("--- exit fork of " + fork.getKey() + " for " +
                     mojoExecId + " ---");
      }
    }
