...
public class MojoDescriptorCreator {
  ...
  @Inject private PluginPrefixResolver pluginPrefixResolver;
  ...
  public MojoDescriptor getMojoDescriptor(String task, MavenSession session,
    ...
    if (numTokens >= 4) {
      ...
      // This won't be valid, but it constructs something easy to read in the
      // error message
      ...
      // This is the case where someone has executed a single goal from the
      // command line of the form:
      //
      // mvn remote-resources:process
      //
      // From the metadata stored on the server which has been created as part
      // of a standard Maven plugin deployment we will find the right
      // PluginDescriptor from the remote repository.
    ...
    // metadata for this plugin and see what's specified as the latest release.
    //
  ...
  public Plugin findPluginForPrefix(String prefix, MavenSession session)
    ...
    PluginPrefixRequest prefixRequest =
        new DefaultPluginPrefixRequest(prefix, session);
    PluginPrefixResult prefixResult =
        pluginPrefixResolver.resolve(prefixRequest);
  ...
  private void resolvePluginVersion(Plugin plugin, MavenSession session,
                                    MavenProject project)
  ...
  private void injectPluginDeclarationFromProject(Plugin plugin,
    ...
    if (pluginInPom == null && project.getPluginManagement() != null) {
      pluginInPom =
          findPlugin(plugin, project.getPluginManagement().getPlugins());
    }
