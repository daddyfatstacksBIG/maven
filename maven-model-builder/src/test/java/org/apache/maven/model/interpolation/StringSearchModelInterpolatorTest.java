...
public class StringSearchModelInterpolatorTest
  ...
  createInterpolator(org.apache.maven.model.path.PathTranslator translator) {
    return this.interpolator;
  }

  protected ModelInterpolator createInterpolator() { return this.interpolator; }
  ...
  public void testNotInterpolateObjectWithFile() throws Exception {
    ...
    ObjectWithNotInterpolatedFile obj =
        new ObjectWithNotInterpolatedFile(baseDir);
  ...
  private static Object[] readFieldsArray(Object o)
  ...
  private static Map<Class<?>, ?> getCachedEntries()
      throws NoSuchFieldException, IllegalAccessException {
    Field field =
        StringSearchModelInterpolator.class.getDeclaredField("CACHED_ENTRIES");
    field.setAccessible(true);
  ...
  public void testNotInterpolateFile() throws Exception {
    ...
    SimpleProblemCollector collector = new SimpleProblemCollector();
    interpolator.interpolateObject(baseDir, model, new File("."), config,
                                   collector);
    assertProblemFree(collector);
  ...
  public void testConcurrentInterpolation() throws Exception {
    ...
    for (Future<SimpleProblemCollector> result : futures) {
      SimpleProblemCollector problemCollector =
          result.get(); // ArrayIndexOutOfBoundsException are typical indication
                        // of threading issues
  ...
  private static final class ObjectWithStringArrayField {
    ...
    public ObjectWithStringArrayField(String[] values) { this.values = values; }
  }

  private static final class ObjectWithListField {
    private final List<?> values;

    public ObjectWithListField(List<?> values) { this.values = values; }
  }

  private static final class ObjectWithMapField {
    private final Map<?, ?> values;

    public ObjectWithMapField(Map<?, ?> values) { this.values = values; }
  }

  private static final class ObjectWithNotInterpolatedFile {
    private final File f;

    ObjectWithNotInterpolatedFile(File f) { this.f = f; }
  }
  ...
  public void testFinalFieldsExcludedFromInterpolation() {
    ...
    interpolator.interpolateObject(new ClassWithFinalField(), new Model(), null,
                                   request, problems);
