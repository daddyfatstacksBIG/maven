...
public class StringSearchModelInterpolator
  ...
  private interface InnerInterpolator { String interpolate(String value); }
  ...
  void interpolateObject(Object obj, Model model, File projectDir,
                         ModelBuildingRequest config,
                         ModelProblemCollector problems) {
    ...
    PrivilegedAction<Object> action =
        new InterpolateObjectAction(obj, innerInterpolator, problems);
    AccessController.doPrivileged(action);
  }

  private InnerInterpolator
  createInterpolator(List<? extends ValueSource> valueSources,
                     List<? extends InterpolationPostProcessor> postProcessors,
  ...
  private static final class InterpolateObjectAction
    ...
    InterpolateObjectAction(Object target, InnerInterpolator interpolator,
                            ModelProblemCollector problems) {
    ...
    private static void evaluateArray(Object target,
                                      InterpolateObjectAction ctx) {
    ...
    private static class CacheItem {
      ...
      private boolean isQualifiedForInterpolation(Field field,
                                                  Class<?> fieldType) {
        if (Map.class.equals(fieldType) &&
            "locations".equals(field.getName())) {
          return false;
        }
          ...
          interpolateObjectAction.problems.add(
              new ModelProblemCollectorRequest(Severity.ERROR, Version.BASE)
                  .setMessage("Failed to interpolate field3: " + field +
                              " on class: " + field.getType().getName())
    ...
    static final class StringField extends CacheField {
      StringField(Field field) { super(field); }
    ...
    static final class ListField extends CacheField {
      ListField(Field field) { super(field); }
    ...
    static final class MapField extends CacheField {
      MapField(Field field) { super(field); }
