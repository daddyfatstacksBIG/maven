...
interface ModelCacheTag<T> {
   ...
   * retrieved data can be mutated after the cache is queried but the state of
   * the cache must not change so we need to make a copy.
   *
  ...
  ModelCacheTag<ModelData> RAW = new ModelCacheTag<ModelData>() {
    ...
    public ModelData intoCache(ModelData data) {
      ...
      return new ModelData(data.getSource(), model, data.getGroupId(),
                           data.getArtifactId(), data.getVersion());
    }
