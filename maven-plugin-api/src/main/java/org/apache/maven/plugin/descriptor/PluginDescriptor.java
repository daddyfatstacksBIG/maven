...
public class PluginDescriptor
    extends ComponentSetDescriptor implements Cloneable {
  ...
  public void addMojo(MojoDescriptor mojoDescriptor)
    ...
    // this relies heavily on the equals() and hashCode() for
    // ComponentDescriptor, which uses role:roleHint for identity...and roleHint
    // == goalPrefix:goal. role does not vary for Mojos.
    ...
    if (existing != null) {
      throw new DuplicateMojoDescriptorException(
          getGoalPrefix(), mojoDescriptor.getGoal(),
  ...
  public String getPluginLookupKey() { return groupId + ":" + artifactId; }
  ...
  public String getGoalPrefix() { return goalPrefix; }

  public void setGoalPrefix(String goalPrefix) { this.goalPrefix = goalPrefix; }
  ...
  public boolean isInheritedByDefault() { return inheritedByDefault; }
   ...
   * artifacts shadowed by the Maven core realm like {@code maven-project}.
   *
  ...
  public boolean equals(Object object) {
    ...
    return object instanceof PluginDescriptor &&
        getId().equals(((PluginDescriptor)object).getId());
  }
  ...
  public MojoDescriptor getMojo(String goal) {
    ...
    // TODO could we use a map? Maybe if the parent did that for components too,
    // as this is too vulnerable to changes above not being propagated to the
    // map
  ...
  public void setIntroducedDependencyArtifacts(
      Set<Artifact> introducedDependencyArtifacts) {
  ...
  public Artifact getPluginArtifact() { return pluginArtifact; }
  ...
  private InputStream getDescriptorStream(String descriptor)
      throws IOException {
    File pluginFile =
        (pluginArtifact != null) ? pluginArtifact.getFile() : null;
    if (pluginFile == null) {
      throw new IllegalStateException(
          "plugin main artifact has not been resolved for " + getId());
    }

    if (pluginFile.isFile()) {
      try {
        return new URL("jar:" + pluginFile.toURI() + "!/" + descriptor)
            .openStream();
