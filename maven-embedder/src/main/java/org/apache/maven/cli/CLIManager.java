...
public class CLIManager {
  ...
  public CLIManager() {
    ...
    options.addOption(Option.builder(Character.toString(HELP))
                          ...
                          .build());
    options.addOption(
        Option.builder(Character.toString(ALTERNATE_POM_FILE))
            .longOpt("file")
            .hasArg()
            .desc(
                "Force the use of an alternate POM file (or directory with pom.xml)")
            .build());
    options.addOption(Option.builder(Character.toString(SET_SYSTEM_PROPERTY))
                          .longOpt("define")
                          .hasArg()
    ...
    options.addOption(Option.builder(Character.toString(NON_RECURSIVE))
                          ...
                          .build());
    options.addOption(
        Option.builder(Character.toString(UPDATE_SNAPSHOTS))
    ...
    options.addOption(Option.builder(Character.toString(ACTIVATE_PROFILES))
                          ...
                          .build());
    options.addOption(
        Option.builder(Character.toString(BATCH_MODE))
            .longOpt("batch-mode")
    ...
    options.addOption(Option.builder(SUPRESS_SNAPSHOT_UPDATES)
                          ...
                          .build());
    options.addOption(
        Option.builder(Character.toString(CHECKSUM_FAILURE_POLICY))
            .longOpt("strict-checksums")
            .desc("Fail the build if checksums don't match")
            .build());
    options.addOption(
        Option.builder(Character.toString(CHECKSUM_WARNING_POLICY))
            .longOpt("lax-checksums")
            .desc("Warn if checksums don't match")
            .build());
    options.addOption(
        Option.builder(Character.toString(ALTERNATE_USER_SETTINGS))
            .longOpt("settings")
    ...
    options.addOption(Option.builder(ALTERNATE_GLOBAL_SETTINGS)
                          ...
                          .build());
    options.addOption(
        Option.builder(Character.toString(ALTERNATE_USER_TOOLCHAINS))
            .longOpt("toolchains")
    ...
    options.addOption(Option.builder(ALTERNATE_GLOBAL_TOOLCHAINS)
                          ...
                          .build());
    options.addOption(
        Option.builder(FAIL_ON_SEVERITY)
            .longOpt("fail-on-severity")
            .desc(
                "Configure which severity of logging should cause the build to fail")
            .hasArgs()
            .build());
    ...
    options.addOption(
        Option.builder(PROJECT_LIST)
            ...
            .build());
    options.addOption(
        Option.builder(ALSO_MAKE)
            .longOpt("also-make")
            .desc(
                "If project list is specified, also build projects required by the list")
            .build());
    options.addOption(
        Option.builder(ALSO_MAKE_DEPENDENTS)
            .longOpt("also-make-dependents")
            .desc(
                "If project list is specified, also build projects that depend on projects on the list")
            .build());
    options.addOption(
        Option.builder(LOG_FILE)
            .longOpt("log-file")
            .hasArg()
            .desc(
            ...
            .build());
    options.addOption(
        Option.builder(Character.toString(SHOW_VERSION))
    ...
    options.addOption(
        Option.builder(THREADS)
            ...
            .build());
    options.addOption(
        Option.builder(LEGACY_LOCAL_REPOSITORY)
    ...
    options.addOption(Option.builder(BUILDER)
                          ...
                          .build());
    options.addOption(
        Option.builder(NO_TRANSFER_PROGRESS)
            .longOpt("no-transfer-progress")
            .desc(
                "Do not display transfer progress when downloading or uploading")
            .build());

    // Adding this back in for compatibility with the verifier that hard codes
    // this option.
    options.addOption(
        Option.builder("npr")
  ...
  public void displayHelp(PrintStream stdout) {
    ...
    formatter.printHelp(pw, HelpFormatter.DEFAULT_WIDTH,
                        ...
                        "\nOptions:", options, HelpFormatter.DEFAULT_LEFT_PAD,
