...
public class MavenCliTest {
  ...
  public void testCalculateDegreeOfConcurrencyWithCoreMultiplier() {
    ...
    // -T2.2C
    assertEquals((int)(cores * 2.2),
                 cli.calculateDegreeOfConcurrencyWithCoreMultiplier("C2.2"));
    // -TC2.2
    assertEquals((int)(cores * 2.2),
  ...
  public void testMavenConfig() throws Exception {
    ...
    assertEquals("multithreaded",
                 request.commandLine.getOptionValue(CLIManager.BUILDER));
  ...
  testMVNConfigurationDefinedPropertiesCanBeOverwrittenViaCommandLine()
    ...
    System.setProperty(
        ...
        new File("src/test/projects/mavenConfigProperties").getCanonicalPath());
    CliRequest request =
        new CliRequest(new String[] {"-Drevision=8.1.0"}, null);
  ...
  public void testMVNConfigurationCLIRepeatedPropertiesLastWins()
    ...
    CliRequest request = new CliRequest(
        new String[] {"-Drevision=8.1.0", "-Drevision=8.2.0"}, null);
  ...
  public void testStyleColors() throws Exception {
    ...
    MessageUtils.setColorEnabled(false);
    request = new CliRequest(new String[] {"-Dstyle.color=always", "-B", "-l",
                                           "target/temp/mvn.log"},
                             null);
    cli.cli(request);
    cli.properties(request);
    ...
    try {
      MessageUtils.setColorEnabled(false);
      request = new CliRequest(new String[] {"-Dstyle.color=maybe", "-B", "-l",
                                             "target/temp/mvn.log"},
                               null);
      cli.cli(request);
      cli.properties(request);
    ...
    MavenCli customizedMavenCli = new MavenCli() {
      ...
      protected void customizeContainer(PlexusContainer container) {
        ...
        ((DefaultPlexusContainer)container)
            .addPlexusInjector(Collections.<PlexusBeanModule>emptyList(),
                               new Module() {
                                 public void configure(final Binder binder) {
                                   binder.bind(EventSpyDispatcher.class)
