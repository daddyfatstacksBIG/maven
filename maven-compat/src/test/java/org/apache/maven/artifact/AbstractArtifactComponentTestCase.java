...
public abstract class AbstractArtifactComponentTestCase extends PlexusTestCase {
  ...
  protected ArtifactRepository badLocalRepository() throws Exception {
    String path =
        "target/test-repositories/" + component() + "/bad-local-repository";
  ...
  protected String getRepositoryLayout() { return "default"; }

  protected ArtifactRepository localRepository() throws Exception {
    String path =
        "target/test-repositories/" + component() + "/local-repository";
  ...
  protected ArtifactRepository remoteRepository() throws Exception {
    String path =
        "target/test-repositories/" + component() + "/remote-repository";
    ...
    return artifactRepositoryFactory.createArtifactRepository(
        "test", "file://" + f.getPath(), repoLayout,
  ...
  protected void createArtifact(Artifact artifact,
    ...
    try (Writer writer = new OutputStreamWriter(
             new FileOutputStream(artifactFile), StandardCharsets.ISO_8859_1)) {
  ...
  protected void deleteArtifact(Artifact artifact,
    ...
    if (artifactFile.exists()) {
      if (!artifactFile.delete()) {
        throw new IOException("Failure while attempting to delete artifact " +
                              artifactFile);
      }
    }
  }
  ...
  protected RepositorySystemSession initRepoSession() throws Exception {
    ...
    transformer = new ChainedDependencyGraphTransformer(
        transformer, new JavaDependencyContextRefiner());
    session.setDependencyGraphTransformer(transformer);

    LocalRepository localRepo =
        new LocalRepository(localRepository().getBasedir());
