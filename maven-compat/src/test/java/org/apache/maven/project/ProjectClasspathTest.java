...
public class ProjectClasspathTest extends AbstractMavenProjectTestCase {
  ...
  public void testProjectClasspath() throws Exception {
    File f = getFileForClasspathResource(
        dir + "project-with-scoped-dependencies.xml");
    ...
    // check all transitive deps of a provided dependency are provided scope,
    // except for test
    ...
    // check all transitive deps of a runtime dependency are runtime scope,
    // except for test
    ...
    // check all transitive deps of a compile dependency are compile scope,
    // except for runtime and test
    checkGroupIdScope(project, "compile", "maven-test-compile");
    artifact = getArtifact(project, "maven-test-compile", "scope-runtime");
    assertEquals("Check scope", "runtime", artifact.getScope());

    // check all transitive deps of a default dependency are compile scope,
    // except for runtime and test
  ...
  private void checkGroupIdScope(MavenProject project, String scopeValue,
                                 String groupId) {
    Artifact artifact;
  ...
  private void checkArtifactIdScope(MavenProject project, String scope,
                                    String scopeValue) {
  ...
  private Artifact getArtifact(MavenProject project, String groupId,
                               String artifactId) {
