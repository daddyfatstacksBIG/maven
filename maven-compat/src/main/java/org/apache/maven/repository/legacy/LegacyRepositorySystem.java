...
public class LegacyRepositorySystem implements RepositorySystem {
  ...
  @Requirement private PlexusContainer plexus;
  ...
  public Artifact createArtifact(String groupId, String artifactId,
    ...
    return artifactFactory.createBuildArtifact(groupId, artifactId, version,
                                               packaging);
  }
  ...
  public Artifact createProjectArtifact(String groupId, String artifactId,
    ...
    return artifactFactory.createProjectArtifact(groupId, artifactId,
                                                 metaVersionId);
  }
  ...
  public Artifact createPluginArtifact(Plugin plugin) {
    ...
    return artifactFactory.createPluginArtifact(
  ...
  private String canonicalFileUrl(String url) throws IOException {
    ...
    // We want to eliminate any relative path nonsense and lock down the path so
    // we need to fully resolve it before any sub-modules use the path. This can
    // happen when you are using a custom settings.xml that contains a relative
  ...
  private org.apache.maven.settings.Proxy
  getProxy(ArtifactRepository repository,
    ...
    if (proxies != null && repository.getProtocol() != null) {
      for (org.apache.maven.settings.Proxy proxy : proxies) {
        if (proxy.isActive() &&
            repository.getProtocol().equalsIgnoreCase(proxy.getProtocol())) {
  ...
  public void retrieve(ArtifactRepository repository, File destination,
    ...
    try {
      wagonManager.getRemoteFile(
          repository, destination, remotePath,
      ...
      throw new ArtifactTransferFailedException(
          getMessage(e, "Error transferring artifact."), e);
  ...
  public void publish(ArtifactRepository repository, File source,
                      String remotePath,
                      ArtifactTransferListener transferListener)
      throws ArtifactTransferFailedException {
    try {
      wagonManager.putRemoteFile(
          repository, source, remotePath,
          TransferListenerAdapter.newAdapter(transferListener));
    } catch (org.apache.maven.wagon.TransferFailedException e) {
      throw new ArtifactTransferFailedException(
          getMessage(e, "Error transferring artifact."), e);
  ...
  public ArtifactRepository buildArtifactRepository(Repository repo)
    ...
    if (repo != null) {
      ...
      return createArtifactRepository(id, url, getLayout(repo.getLayout()),
                                      snapshots, releases);
    } else {
      return null;
    }
  ...
  createRepository(String url, String repositoryId, boolean releases,
    ...
    return createArtifactRepository(repositoryId, url, null, snapshotsPolicy,
                                    releasesPolicy);
  }

  public ArtifactRepository
  createArtifactRepository(String repositoryId, String url,
   ...
   * system needs to retain the id of this layout so that the content type of
   * the remote repository can still be accurately described.
   */
