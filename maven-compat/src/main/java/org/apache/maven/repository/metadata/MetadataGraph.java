...
public class MetadataGraph {
   ...
   * init graph
   */
  public MetadataGraph(int nVertices) { init(nVertices, 2 * nVertices); }
  public MetadataGraph(int nVertices, int nEdges) { init(nVertices, nEdges); }
  //------------------------------------------------------------------------
  /**
  ...
  private void processTreeNodes(MetadataGraphVertex parentVertex,
                                MetadataTreeNode node, int depth, int pomOrder)
      throws MetadataResolutionException {
    if (node == null) {
      return;
    }

    MetadataGraphVertex vertex =
        new MetadataGraphVertex(node.md, versionedVertices, scopedVertices);
    ...
    if (parentVertex != null) // then create the edge
    {
      ...
      MetadataGraphEdge e =
          new MetadataGraphEdge(md.version, md.resolved, md.artifactScope,
                                md.artifactUri, depth, pomOrder);
  ...
  private void checkVertices() { checkVertices(DEFAULT_VERTICES); }
  ...
  public MetadataGraph addEdge(MetadataGraphVertex vFrom,
                               MetadataGraphVertex vTo, MetadataGraphEdge e)
      throws MetadataResolutionException {
    checkVertex(vFrom);
    checkVertex(vTo);
    ...
    List<MetadataGraphEdge> exList =
        excidentEdges.computeIfAbsent(vFrom, k -> new ArrayList<>());
    ...
    List<MetadataGraphEdge> inList =
        incidentEdges.computeIfAbsent(vTo, k -> new ArrayList<>());
  ...
  //------------------------------------------------------------------------
  public MetadataGraphVertex getEntry() { return entry; }

  public void setEntry(MetadataGraphVertex entry) { this.entry = entry; }

  public TreeSet<MetadataGraphVertex> getVertices() { return vertices; }
  ...
  public boolean isVersionedVertices() { return versionedVertices; }

  public void setVersionedVertices(boolean versionedVertices) {
    this.versionedVertices = versionedVertices;
  }

  public boolean isScopedVertices() { return scopedVertices; }
  ...
  public ArtifactScopeEnum getScope() { return scope; }

  public void setScope(ArtifactScopeEnum scope) { this.scope = scope; }
